...
from comprex.scheduler import blockwise_shuffle, geom_responses
...
def generate_extinction_trials(trial: int, ext: int) -> List[bool]:
    ret = [False for _ in range(trial)]
    where_ext = trial // ext
    for i in range(ext):
        ret[where_ext * i] = True
    return blockwise_shuffle(ret, where_ext)
...
async def stimulate(agent: Agent, ino: Arduino, expvars: Experimental) -> None:
    ...
    extinction_response = max(required_responses) * 3 + 10
    extinction_duration = expvars.get("ext-duration", 30.)
    num_extinction = expvars.get("num-extinction", 4)
    extinction_trials = generate_extinction_trials(trial, num_extinction)
    ...
    try:
        ...
        while agent.working():
            for i in range(trial):
                ...
                if extinction_trials[i]:
                    t, _ = timestamp(None)
                    for _ in range(extinction_response):
                        await agent.recv()
                        t_, _ = timestamp(None)
                        if t_ - t >= extinction_duration:
                            break
                else:
