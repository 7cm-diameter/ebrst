...
function run_baseline(parameters::Tuple{Float64, Float64, Float64})::DataFrame
  p, αp, αn = parameters
  ...
  response_strength(agent)

  DataFrame([p αp αn agent.q agent.ϵ response_strength(agent)],
            ["p", "alpha.p", "alpha.n", "q", "epsilon", "response"])
end


Αp = collect(0.01:0.01:1.)
Αn = collect(0.01:0.01:0.5)
P = collect(0.00:0.01:1.)

parameter_sets = Iterators.product(P, Αp, Αn) |> collect |> x -> reshape(x, 1, :)
results = map(params -> run_baseline(params), parameter_sets) |> x -> reduce(vcat, x)
CSV.write("./data/fig1H.csv", results)
